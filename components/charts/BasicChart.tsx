...
const basicChartOptions = (chartData: Project[]): ReactEChartsProps["option"] => (
	{
		...
		series: 
			chartData.map( project => ({
				...
				// @ts-ignore
				data: [[project.createdOn,0], ...project.enrollRequests.map( (request,index) => [request.createdOn,index+1]),[new Date().toISOString().slice(0, 10), project.enrollRequests.length]]
			}))
			// {
			// 	name: "Union Ads",
			// 	type: "line",
