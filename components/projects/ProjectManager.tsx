...
export const ProjectManager = ({ 
  ...
  const [initialPriorityList, setInitialPriorityList] = useState(projectsProp.map(project => ({ id: project.id, priority: project.priority})))
  ...
  const refreshData = async () => {
    ...
    if (res.status == 200) {
      ...
      setInitialPriorityList(projects.map(project => ({ id: project.id, priority: project.priority})))
    }
    setLoading(false)
  };
  ...
  const handleUpdateProjectsPriority = useCallback((updatedProjects: Project[]) => {
    const priority: Priority[] = updatedProjects.map((project,index) => ({ id: project.id, priority: index}))
        ...
        <><Grid container mt={2} mb={2} spacing={2}>
            ...
            <Grid>
                <StyledButton
                    ...
                    disabled={!priorityList || JSON.stringify(priorityList) === JSON.stringify(initialPriorityList)}
                >
